...
STATS_PORT=$(docker exec "$CONTAINER" python3 -c "
...
echo "timestamp,elapsed_min,vm_rss_kb,vm_data_kb,cgroup_bytes,gc_collected_total,gc_uncollectable_total,gc_collections_total,process_rss_bytes,process_cpu_seconds,open_fds" \
...
try:
    ...
    print(f'{gc_collected:.0f},{gc_uncollectable:.0f},{gc_collections:.0f},{rss},{cpu},{fds}')
except:
    print('0,0,0,0,0,0')
" 2>/dev/null)
    fi
